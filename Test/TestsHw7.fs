   ...
   [<Properties(MaxTest = 100)>]
    ...
    type mapTest() =
        ...
        member _.intTest (matr: int array2d) = 
            ...
            let qmatr2d = toArray2d mapedMatr qsize
        ...
        member _.floatTest (size: uint) = 
            let matr: float32 array2d array = Gen.sample 1 (PropertyQtreeTest.matrixGenerator (int size) (int size) (Gen.elements {-infinityf .. infinityf}))
            ...
            let qmatr2d = toArray2d mapedMatr qsize
            let actmatr = Array2D.create heigth wide 0f
            for i in 0 .. heigth - 1 do
                for j in 0 .. wide - 1 do
                    actmatr[i, j] <- qmatr2d[i, j]
            Assert.Equal(expmatr, actmatr)
    ...
    type map2Test()=
        ...
        member _.intTest (size: uint) =
            let matr = Gen.sample 2 (PropertyQtreeTest.matrixGenerator (int size) (int size) (Gen.choose (-100000, 100000)))
            ...
            let qmatr2d = toArray2d actqmatr  qsize1
        ...
        member _.charTest (size: uint) =
            let matr = Gen.sample 2 (PropertyQtreeTest.matrixGenerator (int size) (int size) (Gen.elements {' ' .. '~'}))
            let matr11d = PropertyQtreeTest.toArray matr.[0]
            let matr21d = PropertyQtreeTest.toArray matr.[1]
            let expmatr = Array.map2 (fun x y -> x + y) matr11d matr21d    
            let qmatr1, qsize1 = PropertyQtreeTest.createQtree matr.[0]
        ...
        [<Property>]    
        member _.floatTest (size: uint) =
            let matr: float32 array2d array = Gen.sample 2 (PropertyQtreeTest.matrixGenerator (int size) (int size) (Gen.elements {-infinityf .. infinityf}))
            let matr11d = PropertyQtreeTest.toArray matr.[0]
            let matr21d = PropertyQtreeTest.toArray matr.[1]
            let expmatr = Array.map2 (fun x y -> x + y) matr11d matr21d    
            let qmatr1, qsize1 = PropertyQtreeTest.createQtree matr.[0]
            ...
            let actqmatr = map2 (+) qmatr1 qmatr2
            let qmatr2d = toArray2d actqmatr  qsize1
            let actmatr2d = Array2D.create (int size) (int size) 0f
            for i in 0 .. int size - 1 do
                for j in 0 .. int size - 1 do
                    actmatr2d[i, j] <- qmatr2d[i, j]
            let actmatr = PropertyQtreeTest.toArray actmatr2d
    ...
    type multTest() =
        ...
        member _.charTest (size: uint) =
            ...
            let qmatr2d = toArray2d actqmatr qsize1
            let actmatr = Array2D.create (int size) (int size) ' '
            for i in 0 .. int size - 1 do
                for j in 0 .. int size - 1 do
                    actmatr[i, j] <- qmatr2d[i, j]
            Assert.Equal(expmatr, actmatr)
        ...
        [<Property>]    
        member _.floatTest (size: uint) =
            let matr = Gen.sample 2 (PropertyQtreeTest.matrixGenerator (int size) (int size) (Gen.elements {-infinityf .. infinityf}))
            let expmatr= PropertyQtreeTest.multMatr (+) ( * ) matr.[0] matr.[1]
            let qmatr1, qsize1 = PropertyQtreeTest.createQtree matr.[0]
            let qmatr2, qsize2 = PropertyQtreeTest.createQtree matr.[1]
            let actqmatr = multiplyMatrix ( * ) (+) qmatr1 qsize1 qmatr2 qsize2
            let qmatr2d = toArray2d actqmatr qsize1
            let actmatr = Array2D.create (int size) (int size) 0f
            for i in 0 .. int size - 1 do
                for j in 0 .. int size - 1 do
                    actmatr[i, j] <- qmatr2d[i, j]
            Assert.Equal(expmatr, actmatr)